...
export default function LandingPage() {
  ...
  useEffect(() => {
    if (id) fetch(`/api/product?id=${id}`).then(r => r.json()).then(setProduct);
  }, [id]);
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '0 auto', padding: '1rem' }}>
      {product.images?.[0] && <img src={product.images[0]} alt="" style={{ width: '100%' }} />}
      <h2>{product.name}</h2>
      <p>⭐ {product.rating} | Отзывов: {product.feedbacks}</p>
      <p style={{ fontSize: '2rem', fontWeight: 'bold' }}>{product.price} ₽</p>
      <a href={buyUrl} target="_blank" rel="noopener" style={{
        display: 'inline-block', background: '#f15a29', color: 'white', padding: '1rem 2rem',
